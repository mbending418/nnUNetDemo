...
MODALITY_FOLDERS = {"CT": "ct_data", "T2": "mr_data"}
...
def sort_amos22(input_folder: str, subfolders_to_sort: tuple[str, ...] = subfolders):
    ...
    if os.path.isdir(os.path.join(input_folder, MODALITY_FOLDERS["CT"])):
        raise Exception(
            f"CT Data directory already present: {os.path.join(input_folder, MODALITY_FOLDERS["CT"])}"
        )
    if os.path.isdir(os.path.join(input_folder, MODALITY_FOLDERS["T2"])):
        raise Exception(
            f"MR Data directory already present: {os.path.join(input_folder, MODALITY_FOLDERS["T2"])}"
        )
        ...
        for case in os.listdir(image_input_folder):
            ...
            if is_ct(case):
                output_image_folder = os.path.join(
                    input_folder, MODALITY_FOLDERS["CT"], "imagesTr"
                )
                output_label_folder = os.path.join(
                    input_folder, MODALITY_FOLDERS["CT"], "labelsTr"
                )
            else:
                output_image_folder = os.path.join(
                    input_folder, MODALITY_FOLDERS["T2"], "imagesTr"
                )
                output_label_folder = os.path.join(
                    input_folder, MODALITY_FOLDERS["T2"], "labelsTr"
                )

            shutil.copyfile(
                os.path.join(image_input_folder, case),
                os.path.join(output_image_folder, case),
            )
            shutil.copyfile(
                os.path.join(label_input_folder, case),
                os.path.join(output_label_folder, case),
            )
...
def prepare_amos22(
    ...
    prepare_nifti(
        image_label_folder_pairs=[(image_folder, label_folder)],
